...
Для вывода результата неободимо создать массив, в котором будут храниться результаты логических выражений, а значит этот массив должен быть типа boolean и такого же размера, как размер массива с логическими выражениями, то есть N на M. А также нам требуется вывести количество уникальных правых частей всех логических выражений, поэтому необходимо создать переменную для подсчёта типа int, так как размер массива тоже типа int, а значит количество логических выражений в нем не будет превосходить допустимые значения типа int, но и более маленькие типы данных брать не стоит, так как массив может быть большим.
|              | название переменной | Тип (в Java) | 
|--------------|---------------------|--------------|
| Массив       | `res`               | `boolean`    |
| cnt(Число 1) | `cnt`               | `int`        | 
